...
class ErrsoleMongoDB extends EventEmitter {
  ...
  async init () {
    ...
    await this.deleteExpiredNotificationItems();
  }
  ...
  async insertNotificationItem (notification = {}) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        let formattedPreviousNotification = null;
  ...
  /**
   * Deletes expired notifications based on TTL configuration for MongoDB.
   *
   * @async
  ...
  async deleteExpiredNotificationItems () {
    ...
    try {
      ...
      const expirationTime = new Date(Date.now() - notificationsTTL);

      let deletedRowCount;
      do {
        const result = await this.db.collection(this.notificationsCollectionName)
          .deleteMany({ created_at: { $lt: expirationTime } });

        deletedRowCount = result.deletedCount;
