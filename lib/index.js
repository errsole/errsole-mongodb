...
class ErrsoleMongoDB extends EventEmitter {
  ...
  */
  ...
  async ensureLogsTTL () {
    try {
      ...
      if (result && !result.item) {
        ...
        const logsTTL = result.item.value;
  ...
  async updateLogsCollectionTTL (logsTTL) {
    logsTTL = parseInt(logsTTL) / 1000; // in seconds
    ...
    if (ttlIndex && ttlIndex.expireAfterSeconds !== logsTTL) {
      ...
      await this.db.collection(this.logsCollectionName).createIndex({ timestamp: 1 }, { expireAfterSeconds: logsTTL });
    } else if (!ttlIndex) {
      // If no TTL index exists, create one
      await this.db.collection(this.logsCollectionName).createIndex({ timestamp: 1 }, { expireAfterSeconds: defaultTTL / 1000 });
    }
  }
}
