...
class ErrsoleMongoDB extends EventEmitter {
  ...
  * @returns {Promise<{items: string[]}>} - A Promise that resolves with an object containing a sorted array of unique hostnames.
  *   - `items`: An array of sorted, unique hostnames retrieved from the logs collection.
  * @throws {Error} - Throws an error if the operation fails, such as a database query error.
  */
  ...
  async getLogs (filters = {}) {
    ...
    if (filters.hostname) {
      query.hostname = filters.hostname; // Single hostname filter
    }

    // Multiple hostnames filter
