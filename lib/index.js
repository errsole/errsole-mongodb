...
class ErrsoleMongoDB extends EventEmitter {
  ...
  async getLogs (filters = {}) {
    ...
    const documents = await this.db.collection(this.logsCollectionName).find(query).sort(sortOrder).limit(filters.limit).toArray();
  ...
  */
  ...
  async ensureLogsTTL () {
    try {
      const result = await this.getConfig('logsTTL');
      if (result && !result.item) {
        const setTTLResult = await this.setConfig('logsTTL', defaultTTL.toString());
        if (setTTLResult && setTTLResult.item) {
          await this.updateLogsCollectionTTL(defaultTTL.toString());
        } else {
          console.error('Unable to set logs ttl');
        }
      } else {
        const logsTTL = parseInt(result.item.value);
        await this.updateLogsCollectionTTL(logsTTL);
      }
    } catch (err) {
      console.log(err);
    }
   ...
   * @param {number} logsTTL - The TTL value (in seconds) to set for logs expiration.
   * @returns {Promise<void>} - A Promise that resolves when the logs collection TTL is successfully updated.
  ...
  async updateLogsCollectionTTL (logsTTL) {
    ...
    if (ttlIndex && ttlIndex.expireAfterSeconds !== logsTTL) {
      // Drop the existing TTL index
      await this.db.collection(this.logsCollectionName).dropIndex(ttlIndex.name);
      // Create a new TTL index with logsTTL expiration
      await this.db.collection(this.logsCollectionName).createIndex({ timestamp: 1 }, { expireAfterSeconds: parseInt(logsTTL) });
    } else if (!ttlIndex) {
      // If no TTL index exists, create one
      await this.db.collection(this.logsCollectionName).createIndex({ timestamp: 1 }, { expireAfterSeconds: defaultTTL });
    }
  }
}
